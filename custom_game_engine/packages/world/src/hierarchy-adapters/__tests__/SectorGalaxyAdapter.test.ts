...
import { SectorTierAdapter } from '../SectorTierAdapter.js';
import type { SectorConfig } from '../SectorTierAdapter.js';
import { GalaxyTierAdapter } from '../GalaxyTierAdapter.js';
import type { GalaxyConfig } from '../GalaxyTierAdapter.js';
import { AbstractSystem, AbstractSector, AbstractGalaxy } from '@ai-village/hierarchy-simulator';
...
describe('GalaxyTierAdapter', () => {
  ...
  it('should aggregate population correctly', () => {
    ...
    // Use getTotalPopulation() since adapter recursively aggregates sector + children populations
    const expectedPopulation = testSectors.reduce((sum, s) => sum + s.getTotalPopulation(), 0);
