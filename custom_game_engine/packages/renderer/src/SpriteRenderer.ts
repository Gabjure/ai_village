...
function renderPlantSprite(
  ...
  switch (speciesId) {
    ...
    case 'blueberry-bush':
      // Blueberry bush with blue berries
      // Foliage (dark green)
    ...
    case 'raspberry-bush':
      // Raspberry bush with pink-red berries
      // Foliage (dark green)
      ctx.fillStyle = '#2F4F2F'; // Dark green
      ctx.beginPath();
      ctx.arc(x + size / 2, y + size / 2, size / 3, 0, Math.PI * 2);
      ...
      // Raspberries (pink-red dots)
      ctx.fillStyle = '#E30B5C'; // Raspberry
      for (let i = 0; i < 4; i++) {
        const angle = (i / 4) * Math.PI * 2 + Math.PI / 4;
        const berryX = x + size / 2 + Math.cos(angle) * size * 0.2;
        const berryY = y + size / 2 + Math.sin(angle) * size * 0.2;
    ...
    case 'blackberry-bush':
      // Blackberry bush with dark purple-black berries
      // Foliage (dark green)
      ctx.fillStyle = '#2F4F2F'; // Dark green
      ctx.beginPath();
      ctx.arc(x + size / 2, y + size / 2, size / 3, 0, Math.PI * 2);
      ...
      // Blackberries (dark purple dots)
      ctx.fillStyle = '#2C1F3F'; // Very dark purple
      for (let i = 0; i < 4; i++) {
        const angle = (i / 4) * Math.PI * 2 + Math.PI / 4;
        const berryX = x + size / 2 + Math.cos(angle) * size * 0.2;
        const berryY = y + size / 2 + Math.sin(angle) * size * 0.2;
