...
describe('RaceTemplates', () => {
  ...
  describe('getRacesByRealm', () => {
    ...
    it('should return races native to Veil Wild', () => {
      const veilWildRaces = getRacesByRealm('veil_wild');
      expect(veilWildRaces.length).toBeGreaterThan(0);
      expect(veilWildRaces).toContain(SIDHE_RACE);
      veilWildRaces.forEach(race => {
